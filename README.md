...
- **Pre-signal early-warning layer** (`monitor.py`): the level for a setup is usually known *before* the move, so the monitor surfaces it early so you can rest the order at the broker instead of chasing a late Yahoo-delayed fill. **Which setups can fire early was decided by data, not hope** — a full resting-order backtest of every strategy with the confirmation stripped (`backtest/presignal_sim.py`) showed blind early-firing collapses win rate to **41–47%** for ORB, IB, PDH/PDL and VWAP reversion. *The confirmation is the edge, not the level.* Only two setups survive early-arming, so only two are ARMED:
  - **ARMED (resting bracket): ORB + VWAP bounce.** ORB's edge is the level break (resting buy/sell-**stop** at the OR; WR-neutral when filled at the trigger, see `_presignal_test.py`). VWAP bounce holds **74%** even armed early (`presignal_sim`) because its filter — an established trend — is known in advance; rest a **limit** at VWAP for the pullback (VWAP drifts slowly, so it's stable enough on Yahoo data).
  - **WATCH (early heads-up): IB / PDH / PDL / VWAP reversion.** Confirmation trades (pullback, rejection/retest, fade) — arming them blind tested at ~41% WR, so they stay heads-up only: the monitor flags the level early (within 12 pts) and names the expected setup, and you enter on the engine's **SIGNAL CONFIRMED** panel. The only way to get *these* early without losing WR is a real-time bar feed (Tradovate) so the confirmation arrives on time — not available on Yahoo.
