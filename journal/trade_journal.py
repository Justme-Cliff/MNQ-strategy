...
class TradeJournal:
    ...
    def log_trade(self, data: dict) -> int:
        ...
        with self._connect() as conn:
            cur = conn.execute("""
                INSERT INTO trades (
                    timestamp, strategy, direction, entry_price, stop_price,
                    tp1_price, tp2_price, contracts, risk_dollars, score,
                    score_breakdown, outcome, status, drawdown_remaining,
                    daily_pnl, total_pnl, vix
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ts,
                data.get("strategy"),
